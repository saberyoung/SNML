...
import pandas as pd
import numpy as np
import pyfits,dlt40,os,sys,re,string,subprocess,math,ast
...
if __name__ == "__main__":
    ...
    params = read_default()  
    jd = params['global']['jd']
    area_factor = int(params['global']['area_factor'])
    print '\n\n','#'*50,'\n learning from',jd,'images and scaled the images to',area_factor,'*',area_factor,'pixels\n\n'

    X, y = [], []    
    command = ['select distinct(targetid),classificationid,imglist from ML_candidates']
    print command    
    lista = dlt40.dlt40sql.query(command,dlt40.dlt40sql.conn)
    flag = True
    for line0 in lista:
        imglist = ast.literal_eval(line0['imglist'])       
        for imgid in imglist:                        
            command1 = ['select filename,filepath,xpos,ypos from candidates where id='+str(imgid)]
            lista1 = dlt40.dlt40sql.query(command1,dlt40.dlt40sql.conn)
            if len(lista1)==1:
                img = lista1[0]['filepath']+lista1[0]['filename']                
                cx = lista1[0]['xpos']
                cy = lista1[0]['ypos']
            else:raw_input(lista1)                         
            X1 = read_data.read_single_fit(img,cx,cy,area_factor)
            if len(X1)>0:
                X.append(X1)    
                y.append(line0['classificationid'])             
    X,y = np.array(X), np.array(y, dtype=np.int8)     
